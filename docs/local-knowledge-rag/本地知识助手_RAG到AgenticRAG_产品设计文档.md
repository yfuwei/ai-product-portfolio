...
| V1 | 建立本地 RAG：文档导入、chunk 切分、本地 embedding、Chroma 向量检索、基于上下文回答、来源引用和 Web GUI。 |
...
| 隐私控制 | 微调通常需要上传训练数据或维护更复杂训练环境。 | 本地 RAG 可以把资料、embedding 和向量库留在本机。 |
...
![V1 本地 RAG 架构流程图](diagrams/v1-rag-flow.svg)
...
![V2 Agentic RAG 决策流程图](diagrams/v2-agent-flow.svg)
...
| 用户问题类型 | Route | 例子 |
|---|---|---|
| 涉及本地文档、项目资料、历史记录 | retrieve | “本地 RAG 项目如何介绍？” |
| 需要总结、对比、复盘，但依赖已有资料 | retrieve | “对比本地 RAG 和企业知识库 RAG 的区别。” |
